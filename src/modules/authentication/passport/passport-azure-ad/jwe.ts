...
const createBuffer = (data, encoding) => {
  ...
  if (process.version >= 'v6') {
    ...
    if (typeof data === 'string')
      ...
      return new Buffer(data as Buffer);
  }
};
...
const xor = (a: Buffer, b: Buffer): Buffer => {
  ...
  const c = createBuffer(c1, undefined);
...
const AESKeyUnWrap = (algorithm, wrapped_cek, kek) => {
  ...
  ***************************************************************************/
  let A = C.slice(0, 8);
  const R = [createBuffer(1, undefined)];
  for (let i = 1; i <= n; i++)
  ...
  ***************************************************************************/
  for (let j = 5; j >= 0; j--) {
    for (let i = n; i >= 1; i--) {
      ...
      let B = aes.update(Buffer.concat([xor(A, t), R[i]]), null, 'hex' as Utf8AsciiBinaryEncoding);
      B += aes.final('hex');
      B = createBuffer(B, 'hex') as any;

      // A = MSB(64, B)
      A = B.slice(0, 8);

      // R[i] = LSB(64, B)
      R[i] = B.slice(B.length - 8) as any;
    }
  }
...
const decrypt_AES_CBC_HMAC_SHA2 = (algorithm, key, cipherText, iv, aad, authTag) => {
  ...
  // 3.1 compute aad_len
  const aad_len = createBuffer(8, undefined);
 ...
 * @return{Function}  log: logging function
 */
const decryptContent = (header, cek, cipherText, iv, authTag, aad, log) => {
  if (!header.enc)
...
const decrypt = (jweString, jweKeyStore, log, callback) => {
  /****************************************************************************
  ...
  ***************************************************************************/
  ...
  const aad = createBuffer(parts[0], undefined);
