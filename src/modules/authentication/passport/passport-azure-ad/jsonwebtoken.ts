 ...
 * @callback
 */
const verify = (jwtString, PEMKey, options, callback) => {

  // asynchronous wrapper for callback
  const done = (x = undefined, y = undefined) => {
    return process.nextTick(() => {
      callback.apply(null, [x, y]);
    });
  };
 ...
 */
const generateClientAssertion = (clientID, token_endpoint, privatePEMKey, thumbprint, callback) => {
...
export { verify, generateClientAssertion };
